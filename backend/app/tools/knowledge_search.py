...
async def knowledge_search(query: str, knowledge_ids: str, top_k: int = 5) -> str:
    """从知识库中检索与查询相关的文档内容。当需要查找企业私有文档、本地知识库中的信息时调用此工具。

    Args:
        query: 检索查询文本，描述你想查找的信息。
        knowledge_ids: 知识库ID列表，多个ID用英文逗号分隔（如 "kb_001,kb_002"）。
        top_k: 每个知识库返回的最大文档数量，默认为5。
    ...
    try:
        ...
        result = await RagHandler.query(
            query=query, knowledge_ids=ids, top_k=top_k
        )

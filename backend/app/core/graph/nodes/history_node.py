...
# 全局 SummaryAgent 实例缓存
_summary_agent_cache: Dict[str, SummaryAgent] = {}


def get_or_create_summary_agent(session_id: str) -> SummaryAgent:
    """
    ...
    Returns:
        SummaryAgent 实例
    """
    if session_id not in _summary_agent_cache:
        model = LLMService.get_model(
            api_key=settings.LLM_API_KEY,
            base_url=settings.LLM_BASE_URL,
            model_name=settings.LLM_MODEL
        )
        _summary_agent_cache[session_id] = SummaryAgent(model=model)
        logger.info(f"创建新SummaryAgent实例: {session_id}")
    
    return _summary_agent_cache[session_id]
...
async def history_manager_node(state: ChatState) -> dict:
    ...
    try:
        ...
        compressed_dict = await _compress_with_summary(messages_dict, session_id)
        
        # 第二步：Token长度控制
        if settings.ENABLE_TOKEN_CONTROL:
            compressed_dict = await _control_token_length(compressed_dict, session_id)
...
async def _compress_with_summary(messages_dict: list, session_id: str) -> list:
    """
    ...
    """
    # 获取或创建 SummaryAgent
    summary_agent = get_or_create_summary_agent(session_id)
...
async def _control_token_length(messages_dict: list, session_id: str) -> list:
    """
    控制消息列表的token长度
    
    Args:
        messages_dict: 字典格式的消息列表
        session_id: 会话ID，用于创建SummaryAgent
